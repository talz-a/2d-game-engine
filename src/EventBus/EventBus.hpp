...
public:
    ...
    ~EventBus() { Logger::Log("EventBus deconstructor called!"); }

    // Clear subscribers.
    void Reset() { subscribers.clear(); }

    // Listener subscribes to an event: eventBus->SubscribeToEvent<CollisonEvent>(this,
    // &Game::onCollision);
    ...
    // eventBus->EmitEvent<CollisonEvent>(player, entity);
